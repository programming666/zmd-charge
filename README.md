...
- **拔电**：完整三态动画的「电池模式」—— 文案换成「电池模式」，波纹从胶囊两端**向内收拢**、最终被电标吸走（与插电时向外扩散正好相反），暗示电量在流出
...
| 全局快捷键 | 默认 **`Ctrl + Alt + H`**，在任意界面按下即唤起电量 HUD（真实数据 + 完整三态动画）。设置窗口「通用」页可开关，并点击方框录制新的组合键 |
...
| `--preview-unplug` | 用示例数据播放一次**电池模式**动画（拔电） |
...
│  ├─ HotkeyService.cs      # 全局快捷键（RegisterHotKey + message-only 窗口）
